...
Use **`-i your.target.example.com,`** and **`-e dns_target_host=…`** (must match the inventory host); optional **`-e ansible_user=...`**, **`-e ansible_ssh_private_key_file=...`**, `~/.ssh/config`. On the **target:** **`python3 -m dns_proxmox_audit.static_endpoints`**, then **`python3 -m dns_proxmox_audit.hourly_export`** (journal export for the **current partial hour** through now, so recent DNS is included before merge), then **`python3 -m dns_proxmox_audit.merge_hourly`** (package under **`/usr/local/lib/dns_proxmox_audit/`**, **`PYTHONPATH=/usr/local/lib`**) with `sudo` **`-n`**. **`ansible.builtin.fetch`** (as root on the target) copies **`names-review.txt`**, **`apt-names.txt`**, **`ntp.txt`**, and **`dns-ips.txt`** to the repo defaults **`names-review.txt`**, **`apt-names.txt`**, **`ntp.txt`**, **`dns-ips.txt`** (override with **`dns_audit_names_review`**, **`dns_audit_apt_names`**, **`dns_audit_ntp_list`**, **`dns_audit_dns_ips`**).

Optional **`-e dns_pull_merge_hourly_export_mode=previous`** to run **`hourly_export --previous-hour`** instead (only the last **completed** local hour, like the systemd timer—no live partial hour).
